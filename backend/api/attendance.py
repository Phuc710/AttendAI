...
def get_logs(
    ...
    if class_id:   q += " AND ses.class_id=?"; p.append(class_id)
    ...
    with get_db() as conn:
        ...
        if class_id:   cq += " AND ses.class_id=?"; cp.append(class_id)
...
def logs_by_session(session_id: int):
    with get_db() as conn:
        ...
        """, (session_id,)).fetchall()
    return {"session_id": session_id, "total": len(rows), "logs": [dict(r) for r in rows]}
    ...
    with get_db() as conn:
        rows = conn.execute("""
            SELECT l.id, l.check_in_time, l.check_out_time, l.confidence, l.status,
                   ...
                   ses.session_code, c.class_name, c.subject_code
            FROM attendance_logs l
            JOIN sessions ses ON ses.id = l.session_id
            JOIN classes c ON c.id = ses.class_id
        ...
        """, (user_id,)).fetchall()
    return {"user_id": user_id, "total": len(rows), "logs": [dict(r) for r in rows]}
